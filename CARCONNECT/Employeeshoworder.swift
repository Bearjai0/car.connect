...
class Employeeshoworder: UIViewController ,UITableViewDelegate,UITableViewDataSource{
    ...
    @IBOutlet weak var tel_cus: UILabel!
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell:EmployeeshoworderTableViewCell = self.tableV.dequeueReusableCell(withIdentifier: "cell") as! EmployeeshoworderTableViewCell
        cell.namedata.text = name[indexPath.row]
        return cell
    }
    @IBAction func ok(_ sender: UIButton) {
        let dataurl = "http://it.e-tech.ac.th/carconn/shop/Employeeconfirmorder.php?cus_lic=\(id_car)&idcus_frk=\(id_customer)&idlvd_frk=\(id_serviceProvider)"
    ...
    @IBAction func del(_ sender: Any) {
        let dataurl = "http://it.e-tech.ac.th/carconn/shop/deleteEmployeeconfirmorder.php?cus_lic=\(id_car)&idcus_frk=\(id_customer)&idlvd_frk=\(id_serviceProvider)"
        let allowData = dataurl.addingPercentEncoding(withAllowedCharacters: .urlFragmentAllowed)
            let url = URL(string: allowData!)
            let data = try? Data(contentsOf: url!)
                let stringData = String.init(data: data!, encoding: String.Encoding.utf8)
            print(stringData!)
            if stringData=="Error"{
                    let alert = UIAlertController(title: "แจ้งเตือน", message: "ลบไม่สำเร็จ", preferredStyle: .alert)
                           let ok = UIAlertAction(title: "ตกลง", style: .default, handler: nil)
                           self.present(alert,animated: true)
                           alert.addAction(ok)
            
        }else{
             let alert = UIAlertController(title: "แจ้งเตือน", message: "ลบสำเร็จ", preferredStyle: .alert)
                       let ok = UIAlertAction(title: "ตกลง", style: .default, handler: {(ok:UIAlertAction) in
    ...
    func getData(){
        let dataString =  "http://it.e-tech.ac.th/carconn/shop/Employeeshoworder.php?cus_lic=\(id_car)&idcus_frk=\(id_customer)&idlvd_frk=\(id_serviceProvider)"
